...
def create_output_table(cov_mrp_model,cov_mrp_mc,cov_cm_model,cov_cm_mc,cov_inertia_model,
	...
	np_array_to_latex(cov_cm_model,output_dir + "/" + prefix +"cm_cov",decimals = 5)
	np_array_to_latex(cov_cm_mc,output_dir + "/" + prefix +"cov_cm_mc",decimals = 5)
	np_array_to_latex((cov_cm_model - cov_cm_mc) / cov_cm_mc * 100,output_dir + "/" + prefix +"cov_cm_dev",decimals = 3)

	np_array_to_latex(cov_inertia_model,output_dir + "/" + prefix +"inertia_cov",decimals = 5)
	np_array_to_latex(cov_inertia_mc,output_dir + "/" + prefix +"cov_inertia_mc",decimals = 5)
	np_array_to_latex((cov_inertia_model - cov_inertia_mc) / cov_inertia_mc * 100,output_dir + "/" + prefix +"cov_inertia_dev",decimals = 3)

	np_array_to_latex(cov_dims_model,output_dir + "/" + prefix +"dims_cov",decimals = 5)
	np_array_to_latex(cov_dims_mc,output_dir + "/" + prefix +"cov_dims_mc",decimals = 5)
...
def draw_slice(axis,slices,cm,cov_mc,cov_model,cov_numpy = None,output_dir = None,zoom = False):
	...
	if axis == 0:
		plt.scatter( cm[1,: ], cm[2,: ],marker = ".",color = "lightblue" ,alpha = 0.7)
	elif axis == 1:
		plt.scatter( cm[0,: ],  cm[2,: ],marker = ".",color = "lightblue" ,alpha = 0.7)
		
	elif axis == 2:
		plt.scatter( cm[0,: ], cm[1,: ],marker = ".",color = "lightblue" ,alpha = 0.7)
...
def plot_all_results(input_dir,output_dir = ""):
	...
	cov_moments_mc = (1e3) ** 10 * np.loadtxt(input_dir + "/cov_moments_mc.txt")
	cov_moments_model = (1e3) ** 10 * np.loadtxt(input_dir + "/moments_cov.txt")
	moments = (1e3) ** 5 * np.loadtxt(input_dir + "/moments_spread.txt")
	moments = (moments.T - np.mean(moments,axis = 1)).T

	cov_dims_mc = 1e6 * np.loadtxt(input_dir + "/cov_dims_mc.txt")
	cov_dims_model = 1e6 * np.loadtxt(input_dir + "/dims_cov.txt")
	dims = 1e3 * np.loadtxt(input_dir + "/dims_spread.txt")
	...
	draw_slice(0,slices_x,cm,cov_cm_mc,cov_cm_model,output_dir = output_dir,zoom = True)
	draw_slice(1,slices_y,cm,cov_cm_mc,cov_cm_model,output_dir = output_dir,zoom = True)
	draw_slice(2,slices_z,cm,cov_cm_mc,cov_cm_model,output_dir = output_dir,zoom = True)

	draw_slice(0,slices_x,cm,cov_cm_mc,cov_cm_model,output_dir = output_dir,zoom = False)
	draw_slice(1,slices_y,cm,cov_cm_mc,cov_cm_model,output_dir = output_dir,zoom = False)
	draw_slice(2,slices_z,cm,cov_cm_mc,cov_cm_model,output_dir = output_dir,zoom = False)

	draw_dispersions("dims",0,dims,cov_dims_mc,cov_dims_model,[r"$\delta a$ ($\mathrm{m}$)",r"$\delta b$ ($\mathrm{m}$)",r"$\delta c$ ($\mathrm{m}$)"],output_dir = output_dir)
	draw_dispersions("dims",1,dims,cov_dims_mc,cov_dims_model,[r"$\delta a$ ($\mathrm{m}$)",r"$\delta b$ ($\mathrm{m}$)",r"$\delta c$ ($\mathrm{m}$)"],output_dir = output_dir)
	draw_dispersions("dims",2,dims,cov_dims_mc,cov_dims_model,[r"$\delta a$ ($\mathrm{m}$)",r"$\delta b$ ($\mathrm{m}$)",r"$\delta c$ ($\mathrm{m}$)"],output_dir = output_dir)

	draw_dispersions("moments",0,moments,cov_moments_mc,cov_moments_model,[r"$\delta A$ ($\mathrm{m}^5$)",r"$\delta B$ ($\mathrm{m}^5$)",r"$\delta C$ ($\mathrm{m}^5$)"],output_dir = output_dir)
	draw_dispersions("moments",1,moments,cov_moments_mc,cov_moments_model,[r"$\delta A$ ($\mathrm{m}^5$)",r"$\delta B$ ($\mathrm{m}^5$)",r"$\delta C$ ($\mathrm{m}^5$)"],output_dir = output_dir)
	draw_dispersions("moments",2,moments,cov_moments_mc,cov_moments_model,[r"$\delta A$ ($\mathrm{m}^5$)",r"$\delta B$ ($\mathrm{m}^5$)",r"$\delta C$ ($\mathrm{m}^5$)"],output_dir = output_dir)
