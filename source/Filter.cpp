...
void Filter::get_surface_point_cloud_from_trajectory(
	...
	arma::mat matlab_states;
	matlab_states.load(matlab_file_path);

	// Vector of orbit times
	arma::vec orbit_time = matlab_states.col(0);

	// Matrix of orbit states
	arma::mat orbit_states = matlab_states.cols(1, 6);
	arma::inplace_trans(orbit_states);

	// Matrix of attitude states
	arma::mat attitude_states = matlab_states.cols(7, 9);
	arma::inplace_trans(attitude_states);
	...
	// // An interpolator of the attitude state is created
	Interpolator interpolator_attitude(&orbit_time, &attitude_states);
	Interpolator interpolator_orbit(&orbit_time, &orbit_states);
