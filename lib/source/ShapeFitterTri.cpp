...
bool ShapeFitterTri::fit_shape_batch(unsigned int N_iter,double J,const arma::mat & DS, const arma::vec & X_DS){
	...
	for (unsigned int i = 0; i < this -> pc -> get_size(); ++i){
		...
		Footpoint footpoint;
		footpoint.Ptilde = Ptilde;
		this -> find_footpoint(footpoint);
		if(std::abs(arma::dot(footpoint.n,Ptilde - footpoint.Pbar))< J){
			continue;
		}

		seen_facets.insert(footpoint.element);
		measurement_pairs.push_back(std::make_pair(Ptilde,footpoint));

		if (seen_vertices.find(footpoint.element -> get_control_points() -> at(0)) == seen_vertices.end()){
			seen_vertices[footpoint.element -> get_control_points() -> at(0)] = seen_vertices.size();
		}

		if (seen_vertices.find(footpoint.element -> get_control_points() -> at(1)) == seen_vertices.end()){
			seen_vertices[footpoint.element -> get_control_points() -> at(1)] = seen_vertices.size();
		}

		if (seen_vertices.find(footpoint.element -> get_control_points() -> at(2)) == seen_vertices.end()){
			seen_vertices[footpoint.element -> get_control_points() -> at(2)] = seen_vertices.size();
		}
	...
	for (auto iter = measurement_pairs.begin(); iter < measurement_pairs.end(); ++ iter){
		...
		unsigned int iu = seen_vertices[iter -> second.element ->  get_control_points() -> at(0)];
		unsigned int iv = seen_vertices[iter -> second.element ->  get_control_points() -> at(1)];
		unsigned int iw  = seen_vertices[iter -> second.element ->  get_control_points() -> at(2)];
...
void ShapeFitterTri::find_footpoint(Footpoint & footpoint) const {
	...
	double max_distance = std::abs(arma::dot(closest_facet -> get_normal(),
		footpoint.Ptilde - closest_point -> get_coordinates()));
	...
	for (auto iter = owning_elements.begin();iter != owning_elements.end(); ++iter){
		...
		double u,v;
		arma::vec Ptilde_in_facet = footpoint.Ptilde - arma::dot(n,
			footpoint.Ptilde - closest_point -> get_coordinates()) * (n);
	...
	footpoint.Pbar = (footpoint.Ptilde 
		- arma::dot(footpoint.n,footpoint.Ptilde - closest_facet -> get_center()) * (footpoint.n));


	this -> get_barycentric_coordinates(footpoint.Pbar, u,v,dynamic_cast<Facet *>(closest_facet));
	footpoint.u = u;
	footpoint.v = v;
	footpoint.element = closest_facet;
...
void ShapeFitterTri::get_barycentric_coordinates(const arma::vec & Pbar,double & u, double & v, Facet * facet) const{
...
double ShapeFitterTri::compute_residuals(std::vector<std::pair<arma::vec,Footpoint > > & measurement_pairs) const{
	...
	for (auto iter = measurement_pairs.begin(); iter != measurement_pairs.end(); ++iter){
		...
		C =  iter -> second.element -> get_center();
		n =  iter -> second.element -> get_normal();
...
void ShapeFitterTri::save(std::string path, arma::mat & Pbar_mat) const {
