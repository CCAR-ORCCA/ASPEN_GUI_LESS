...
arma::vec Observations::obs_lidar_range_true(double t,
	...
	arma::vec mrp_BN_true = *args.get_mrp_BN_estimated();
	...
	lidar -> send_flash(args.get_true_shape_model());
...
arma::vec Observations::obs_lidar_range_computed(
	...
	// Setting the small body to its inertial attitude. This should not affect the 
	// measurements at all
	frame_graph -> get_frame(args.get_true_shape_model() -> get_ref_frame_name()) -> set_mrp_from_parent(mrp_BN_estimated);
...
arma::vec Observations::obs_pos_computed(double t,const arma::vec & x,const Args & args){
	return x.rows(0,2);
}


arma::vec Observations::obs_pos_lidar(double t,const arma::vec & x,const Args & args){
	...
	arma::mat R = std::pow(args.get_sd_noise(),2) * arma::eye<arma::mat>(N_mes,N_mes);

	arma::vec x_bar_bar = x;
	int iter = filter.run(10,*args. get_true_pos(),x_bar_bar,times,R);
...
arma::mat Observations::obs_pos_jac(double t,const arma::vec & x,const Args & args){
