...
void PC::transform(const arma::mat & dcm, const arma::vec & x){
	...
	for (unsigned int i = 0; i < this -> kdt_points  -> get_points_normals() -> size(); ++i) {
		...
		pn -> set_normal_coordinates(dcm * pn -> get_normal_coordinates());
...
void PC::construct_normals(arma::vec los_dir) {
	...
	for (unsigned int i = 0; i < this -> kdt_points  -> get_points_normals() -> size(); ++i) {
		...
		std::map<double,std::shared_ptr<PointNormal> > closest_points = this -> get_closest_N_points(this -> kdt_points  -> get_normal_coordinates(i) -> get_point(), N);
		...
		if (arma::dot(n, los_dir) < 0) {
			this -> kdt_points  -> get_points_normals() -> at(i) -> set_normal_coordinates(n);
		}
		else {
			this -> kdt_points  -> get_points_normals() -> at(i) -> set_normal_coordinates(-n);
		}
...
void PC::construct_normals(arma::vec los_dir, double radius) {
	...
	for (unsigned int i = 0; i < size; ++i) {
		...
		// Get the N nearest neighbors to this point
		std::vector<std::shared_ptr<PointNormal> > closest_points;
		closest_points = this -> get_points_in_sphere(this -> kdt_points  -> get_normal_coordinates(i) -> get_point(), radius);
...
void PC::compute_mean_feature_histogram(){
	...
	unsigned int size = this -> kdt_points  -> get_points_normals() -> size();
	unsigned int histo_size = this -> kdt_points  -> get_normal_coordinates(0)-> get_histogram_size();

	this -> mean_feature_histogram = arma::zeros<arma::vec>(histo_size);

	for (unsigned int k = 0; k < size; ++k){
		this -> mean_feature_histogram += this -> kdt_points  -> get_normal_coordinates(k) -> get_descriptor_histogram();
	}
