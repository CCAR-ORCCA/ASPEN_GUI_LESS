...
void ShapeBuilder::run_shape_reconstruction(arma::vec &times ,
	...
	for (unsigned int time_index = 0; time_index < this -> get_filter_arguments() -> get_index_end(); ++time_index) {
		...
		std::cout << "\n################### Index : " << time_index << " / " <<  this -> get_filter_arguments() -> get_index_end() - 1  << ", Time : " << times(time_index) << " / " <<  times( this -> get_filter_arguments() -> get_index_end() - 1) << " ########################" << std::endl;
		...
		if (this -> destination_pc != nullptr && this -> source_pc != nullptr) {
			...
			if (this -> estimated_shape_model != nullptr && time_index != this -> filter_arguments -> get_index_init()){
				...
				shape_fitter.fit_shape_KF(time_index,
					4,1e-5,arma::eye<arma::mat>(3,3), arma::zeros<arma::vec>(3),
